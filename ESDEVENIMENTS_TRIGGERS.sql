...
BEGIN
    ...
    IF NEW.PERSONA.EMAIL NOT REGEXP '^[a-z]{3,}((\.|\_)[a-z]{3,})?@[a-z]{6,}\.[a-z]{2,3}$' THEN
    SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = 'El correu electrònic de la persona és invàlid.';
    END IF;
END$$
DELIMITER ;
...
BEGIN
	DECLARE MESSAGE_TEXT VARCHAR(100);
    IF NEW.PERSONA.DATA_NAIXEMENT <= NOW() OR YEAR(NOW())-YEAR(NEW.PERSONA.DATA_NAIXEMENT)>150  THEN
    SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = 'La data de naixement de la persona és invàlid.';
    END IF;
END$$
DELIMITER ;
...
BEGIN
	DECLARE MESSAGE_TEXT VARCHAR(100);
    IF NEW.TELEFON_PERSONA.PK_TELEFON_PERSONA NOT REGEXP '/^(\(?\+[0-9]{2,3}\)?[-.\s]?)?([0-9]{3})[-.\s]?([0-9]{3})[-.\s]?([0-9]{3})[-.\s]?$/' THEN
	SIGNAL SQLSTATE '45000'
...
BEGIN
	DECLARE MESSAGE_TEXT VARCHAR(100);
	IF NEW.RECINTE.DIRECCIO NOT REGEXP '/^([Cc]+[\./\s])?\s?(.*)\,\s*(\d{1,})(\-?\d*?[A-Z]{1}?)?$/' THEN
    SIGNAL SQLSTATE '45000'
...
BEGIN
	DECLARE MESSAGE_TEXT VARCHAR(100);
	IF NEW.RECINTE.CAPACITAT <= 0 THEN
    SIGNAL SQLSTATE '45000'
...
BEGIN
	DECLARE MESSAGE_TEXT VARCHAR(100);
	IF NEW.ESDEVENIMENT.DATA_INICI >= NEW.ESDEVENIMENT.DATA_FI THEN
    SIGNAL SQLSTATE '45000'
...
BEGIN
	DECLARE MESSAGE_TEXT VARCHAR(100);
    IF NEW.ESDEVENIMENT.PREU < 0.00 THEN
    SIGNAL SQLSTATE '45000'
...
BEGIN
	DECLARE MESSAGE_TEXT VARCHAR(100);
    IF NEW.BUTACA.PK_FILA_BUTACA <=0 OR NEW.BUTACA.PK_NUMERO_BUTACA <=0 THEN
	SIGNAL SQLSTATE '45000'
...
BEGIN
    ...
    IF NEW.RECINTE.AFORAMENT > capacitat_recinte THEN
    SIGNAL SQLSTATE '45000'
...
BEGIN
    ...
    IF NEW.ESDEVENIMENT.OCUPACIO > aforament THEN
    SIGNAL SQLSTATE '45000'
...
BEGIN
	DECLARE capacitat_recinte INT;
    SELECT RECINTE.CAPACITAT INTO capacitat_recinte FROM RECINTE
    WHERE RECINTE.PK_CODI_RECINTE = NEW.ESDEVENIMENT.FK_CODI_RECINTE;
    
    UPDATE ESDEVENIMENT
    SET ESDEVENIMENT.AFORAMENT = capacitat_recinte
    WHERE ESDEVENIMENT.PK_ID_ESDEVENIMENT = NEW.ESDEVENIMENT.PK_ID_ESDEVENIMENT;
END $$
DELIMITER ;
...
BEGIN
    ...
    WHERE NEW.ENTRADA.PK_FK_ID_ENTRADA_ESDEVENIMENT = ESDEVENIMENT.PK_ID_ESDEVENIMENT;
    
    SELECT ESDEVENIMENT.AFORAMENT INTO aforament_esdeveniment FROM ESDEVENIMENT
    WHERE ESDEVENIMENT.PK_ID_ESDEVENIMENT = NEW.ENTRADA.PK_FK_ID_ENTRADA_ESDEVENIMENT;
